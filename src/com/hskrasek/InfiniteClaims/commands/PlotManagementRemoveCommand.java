...
public class PlotManagementRemoveCommand extends IClaimsCommand
{
	...
	public PlotManagementRemoveCommand(InfiniteClaims plugin) 
	{
		...
		this.setPermission("iclaims.plot.manage.removemember", "Remove a member to your plot. If you don't provide a world, your current one will be used.", PermissionDefault.OP);
	}
	...
	{		
		...
		if(sender instanceof Player)
		{
			...
			if(worldName != null)
			{
				...
				{
					icUtils.removeMember(player, "all", plotName, new WorldCreator(worldName).createWorld());
				}
				else
				{
					icUtils.removeMember(player, memberToRemove, plotName, new WorldCreator(worldName).createWorld());
				}
			}
			else
			{
				if(memberToRemove.equalsIgnoreCase("all"))
				{
					icUtils.removeMember(player, "all", plotName, player.getWorld());
				}
				else
				{
					icUtils.removeMember(player, memberToRemove, plotName, player.getWorld());
				}
			}
		}
	}
