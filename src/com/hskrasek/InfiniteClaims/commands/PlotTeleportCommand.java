...
public class PlotTeleportCommand extends IClaimsCommand
{
	...
	public PlotTeleportCommand(InfiniteClaims plugin) 
	{
		...
		this.setCommandUsage(String.format("%s/iclaims plot %s{PLOT} %s-p %s[PLAYER]%s -w %s[WORLD]", 
	...
	public void runCommand(CommandSender sender, List<String> args) 
	{
		...
		if(plugin.DEBUGGING)
		{
			plugin.log.debug("Plot Teleport Debug:");
			plugin.log.debug("Args: " + args.toString());
			plugin.log.debug(String.format("Plot Name: %s | Player Name: %s | World name: %s", plotName, playerName, worldName));
		}
		...
		if(sender instanceof Player)
		{
			...
			if(playerName != null)
			{
				...
				if(worldName != null)
				{
					// Teleport to the specified players plot in the specified world
					icUtils.teleportToOtherPlot(player, playerName, plotName, worldName);
				}
				else
				{
					// Teleport to the specified players plot, in the current world
			...
			else
			{
				...
				{
					// Finally, just teleport the player to their own plot in the world they are currently in
					worldName = player.getLocation().getWorld().getName();
					icUtils.teleportToPlot(player, plotName, worldName);
				}
			}
