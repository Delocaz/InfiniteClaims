...
public class LegacyConversion implements Runnable
{
	...
	public void run() 
	{
		...
		{
			plugin.log.info("Found a InfinitePlots world! Now converting plots for world: " + plotWorld.getName());
			...
			for(String currentPlotId : regions.keySet())
			{
				...
				if(plugin.DEBUGGING)
				{
					plugin.log.debug("Current Plot: " + currentPlotId);
					plugin.log.debug("Plot Owner: " + plotOwner);
					plugin.log.debug("Plot Min.: " + plotMinimum.toString());
					plugin.log.debug("Plot Max.: " + plotMaximum.toString());
				}
				...
				IClaimsPlayer player = new IClaimsPlayer(plotOwner);
				int regionCountTemp = 1;
				final int regionCount = mgr.getRegionCountOfPlayer(wgp.wrapPlayer(player));
				
				if(plugin.DEBUGGING)
				{
					plugin.log.debug(String.format("Plot Home - X: %f Y: %d Z: %f", bottomRight.getX() + (plotSize / 2), y+1, topRight.getZ()));
				}
